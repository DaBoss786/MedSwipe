...
const runtimePlatform = capacitorPlatform || detectPlatformFromUserAgent();

function applyPlatformBodyClasses() {
...
function updateToolbarHeightCSSVar() {
  ...
  const measuredHeight = toolbar.offsetHeight;
  if (measuredHeight > 0) {
    document.documentElement.style.setProperty('--toolbar-height', `${measuredHeight}px`);
  }
}
...
function getStatusBarPlugin() {
  ...
  const pluginInstance = capacitor.Plugins?.StatusBar ?? capacitor.StatusBar ?? null;
  if (!pluginInstance) {
    return null;
  }
  ...
  if (typeof capacitor.isPluginAvailable === 'function') {
    try {
      ...
      console.warn('Unable to verify StatusBar plugin availability:', error);
      // Fall through and try to use the plugin anyway.
    }
  }
...
function waitForStatusBarPlugin(timeoutMs = 3000) {
...
let statusBarConfigurationInFlight = null;

async function configureStatusBarForAndroid() {
  if (runtimePlatform !== 'android') {
    return;
  }

  if (statusBarConfigurationInFlight) {
    return statusBarConfigurationInFlight;
  }

  statusBarConfigurationInFlight = (async () => {
    try {
      const statusBarPlugin = await waitForStatusBarPlugin();
      if (!statusBarPlugin) {
        console.warn('StatusBar plugin not found; skipping Android status bar adjustments.');
        return;
      }
      ...
      const darkStyle =
        window.Capacitor?.StatusBarStyle?.Dark ??
        window.Capacitor?.Plugins?.StatusBarStyle?.Dark ??
        'DARK';
      await statusBarPlugin.setStyle?.({ style: darkStyle });
    } catch (error) {
      console.warn('StatusBar configuration failed:', error);
    } finally {
      statusBarConfigurationInFlight = null;
    }
  })();
...
function setupStatusBarLifecycleHooks() {
  if (typeof window === 'undefined') {
    return;
  }

  configureStatusBarForAndroid();
...
function initializePlatformAwareStyling() {
  applyPlatformBodyClasses();
  updateToolbarHeightCSSVar();
}

function handleDocumentReady() {
  initializePlatformAwareStyling();
  setupStatusBarLifecycleHooks();
}
...
if (typeof window !== 'undefined') {
  window.addEventListener('resize', updateToolbarHeightCSSVar);
}

const isNativeApp = detectNativeApp();
const isIosNativeApp = capacitorPlatform === 'ios';
